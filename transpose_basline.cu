...
{
    int continuous_index = blockIdx.x * blockDim.x + threadIdx.x;   // 源矩阵列索引
    int no_continuous_index = blockIdx.y * blockDim.y + threadIdx.y;    // 源矩阵行索引
    
    if (continuous_index < SOURCE_COLUMN && no_continuous_index < SOURCE_ROW)
    {
        p_des[continuous_index * DESTINATION_COLUMN + no_continuous_index] = p_src[no_continuous_index * SOURCE_COLUMN + continuous_index];
    }
}
